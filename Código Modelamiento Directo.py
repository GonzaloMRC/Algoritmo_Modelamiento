...
def longitud_de_extremo(tipo_extrm,luz,d_al):
  ...
  if (tipo_extrm == "TASE" or tipo_extrm == "TAE"):
    ...
    long = d_al #No deberia ser - 0.5*d_al
  return long
...
d_alambre = d #Copia d en d_alambre
...
E1 = input("Extremo 1: ")  #Podria haber una indicacion de las respuestas esperadas
...
if (d1 > 0):  #Parece que si d1 o d2= 0 considera sin reduccion, no se indica
...
#J = (L2 - (2*L3-pf+p0-2*L1)*nc1/2 - 2*(L1-p0)*nc1 - (2*L3-pf)*(N-2-nc1-nc2) - (2*L3-pf)*nc2)/15(nc1*nc2 + 2*nc2*(N-2-nc1-nc2) + pow(nc2,2))
J = (L2 - (2*L3-pf+p0-2*L1)*nc1/2 - 2*(L1-p0)*nc1 - (2*L3-pf)*(N-2-nc1-nc2) - (2*L3-pf)*nc2)/15*(nc1*nc2 + 2*nc2*(N-2-nc1-nc2) + pow(nc2,2))
...
y_total = y1+y2+y3+y4+y5
 ...
 #Se muestran resultados en terminal
print("P2: ", P2)
print("P4: ", P4)
print("K: ", K)
...
#Ecuaciones por tramo, partiendo del eje "Z" -- #Genera nodos/puntos a lo largo de ciertas secciones de una curva para el modelado, np.linspace genera un arreglo de numeros igualmente espaciado en el rango especificado.
...
#Vectores para las coordenadas--- #Se crean los vectores de coordenadas
coord_x1 = []
coord_y1 = []
coord_z1 = []
coord_x2 = []
...
#Sería bueno volver a preguntar si respuesta no es valida enves de terminar el programa
...
ang_t1_2 = []
for i in ang_t1:
  truncado = round(i,5)#redondeo a 5 decilmales
...
print(list_t1)
pos_1 = list_t1.index(pi) #Busqueda de pi en la lista
...
medio_final = round(t_3 - math.pi,5) #redondeo
pos_2 = list_t3.index(medio_final) #busqueda de la posicion 
